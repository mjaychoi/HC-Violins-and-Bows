-- Migration: add DB-backed idempotency for invoice creation
-- Created: 2026-04-01
...
CREATE INDEX IF NOT EXISTS idx_invoice_idempotency_lookup
  ON public.invoice_idempotency_keys(org_id, user_id, route_key, idempotency_key);
...
BEGIN
  FOR r IN
    ...
    WHERE schemaname = 'public'
      AND tablename = 'invoice_idempotency_keys'
  LOOP
    EXECUTE format(
...
CREATE POLICY invoice_idempotency_keys_select
  ON public.invoice_idempotency_keys
  FOR SELECT
  TO authenticated
  USING (org_id = auth.org_id() AND user_id = auth.uid());

CREATE POLICY invoice_idempotency_keys_insert
  ON public.invoice_idempotency_keys
  FOR INSERT
  TO authenticated
  WITH CHECK (org_id = auth.org_id() AND user_id = auth.uid());

CREATE POLICY invoice_idempotency_keys_update
  ON public.invoice_idempotency_keys
  FOR UPDATE
  TO authenticated
  USING (org_id = auth.org_id() AND user_id = auth.uid())
...
CREATE POLICY invoice_idempotency_keys_delete
  ON public.invoice_idempotency_keys
  FOR DELETE
  TO authenticated
  USING (org_id = auth.org_id() AND user_id = auth.uid());
...
GRANT EXECUTE ON FUNCTION public.create_invoice_atomic_idempotent(TEXT, TEXT, TEXT, JSONB, JSONB) TO authenticated;

...
BEGIN
  ...
  IF v_fk_count = 1 THEN
    SELECT c.conname, c.confdeltype
      INTO v_fk_name, v_delete_action
    FROM pg_constraint c
    JOIN pg_attribute a
      ON a.attrelid = c.conrelid
     AND a.attnum = ANY (c.conkey)
    ...
    IF v_fk_name = 'maintenance_tasks_instrument_id_fkey'
       AND v_delete_action = 'r' THEN
...
BEGIN
  ...
  IF v_fk_count = 1 THEN
    ...
    IF v_fk_name = 'instruments_reserved_by_user_id_fkey'
       AND v_delete_action = 'n' THEN
      NULL;
    ELSE
      EXECUTE format(
...
BEGIN
  ...
  IF v_fk_count = 1 THEN
    ...
    IF v_fk_name = 'instruments_reserved_connection_id_fkey'
       AND v_delete_action = 'n' THEN
      NULL;
    ELSE
      EXECUTE format(

-- Migration: make connection reorder atomic
-- Created: 2026-04-03

CREATE OR REPLACE FUNCTION public.reorder_connections_atomic(
  p_orders JSONB
)
RETURNS VOID
LANGUAGE plpgsql
...
BEGIN
  ...
  LOOP
    IF jsonb_typeof(v_item) <> 'object' THEN
      RAISE EXCEPTION 'Each order must be an object';
    END IF;
...
GRANT EXECUTE ON FUNCTION public.reorder_connections_atomic(JSONB) TO authenticated;

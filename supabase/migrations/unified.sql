...
-- ======================================================
-- 0.1) organizations table (for multi-tenancy)
-- ======================================================
...
DO $$
BEGIN
  -- Select policy: Allow all authenticated users to view organizations
  IF NOT EXISTS (
    SELECT 1
    FROM pg_policies
  ...
  -- Insert policy: Only service role can create organizations
  IF NOT EXISTS (
    SELECT 1
    FROM pg_policies
    WHERE schemaname = 'public'
  ...
  -- Update policy: Only service role can update organizations
  IF NOT EXISTS (
    SELECT 1
    FROM pg_policies
    WHERE schemaname = 'public'
  ...
  -- Delete policy: Only service role can delete organizations
  IF NOT EXISTS (
    SELECT 1
    FROM pg_policies
    WHERE schemaname = 'public'
...
-- ======================================================
-- 3) clients: align columns with clientSchema (idempotent)
-- ======================================================

-- already present in your migration, keep it
ALTER TABLE public.clients
  ADD COLUMN IF NOT EXISTS client_number TEXT;

-- ✅ add missing columns used by clientSchema
ALTER TABLE public.clients
...
-- comments (optional but nice)
COMMENT ON COLUMN public.clients.address IS 'Client address (nullable)';
COMMENT ON COLUMN public.clients.tags IS 'Client tags (text array, default empty)';
COMMENT ON COLUMN public.clients.interest IS 'Client interest (nullable)';
COMMENT ON COLUMN public.clients.note IS 'Client note (nullable)';
COMMENT ON COLUMN public.clients.type IS 'Client type (Musician/Dealer/Collector/Regular)';
COMMENT ON COLUMN public.clients.status IS 'Client status (Active/Browsing/In Negotiation/Inactive)';

-- backfill tags when NULL (important if column existed before without default)
UPDATE public.clients
SET tags = ARRAY[]::text[]
WHERE tags IS NULL;

-- optional: enforce allowed values (matches your Zod enums)
ALTER TABLE public.clients
  DROP CONSTRAINT IF EXISTS clients_type_check;

ALTER TABLE public.clients
  ADD CONSTRAINT clients_type_check
  CHECK (type IS NULL OR type IN ('Musician', 'Dealer', 'Collector', 'Regular'));
...
ALTER TABLE public.clients
  ...
  CHECK (status IS NULL OR status IN ('Active', 'Browsing', 'In Negotiation', 'Inactive'));

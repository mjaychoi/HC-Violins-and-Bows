-- Rollback for 20260804010000_enforce_sale_price_precision_and_maximum.sql
-- Restores create_sale_atomic and update_instrument_sale_transition_atomic
-- to their exact origin/main bodies immediately before the forward migration
-- (20260801160100_reconcile_relationships_on_sale.sql for
-- create_sale_atomic and 20260728153000_sale_transition_certificate_name.sql
-- for update_instrument_sale_transition_atomic) and drops the three new table
-- CHECK constraints.
--
-- WARNING: this reopens both gaps described in the forward migration's
-- header comment: RPC callers (and any other insert path) can once again
-- store sale_price values with more than two decimal places (silently
-- rounded by the NUMERIC(12,2) column instead of rejected) and values above
-- $1,000,000,000 with no maximum enforced below the API layer. The two HTTP
-- routes (PATCH /api/instruments, POST/PATCH /api/sales) still enforce both
-- rules via src/utils/salePriceRules.ts regardless of this rollback — only
-- the database-level defense-in-depth is removed.

ALTER TABLE public.sales_history DROP CONSTRAINT IF EXISTS sales_history_undo_refund_sign_check;
ALTER TABLE public.sales_history DROP CONSTRAINT IF EXISTS sales_history_refund_sign_check;
ALTER TABLE public.sales_history DROP CONSTRAINT IF EXISTS sales_history_sale_price_max_magnitude;

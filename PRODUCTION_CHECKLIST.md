...
- [x] 보안 헤더 적용: `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, `Strict-Transport-Security` (Vercel 자동)
  - `next.config.ts`와 `vercel.json`에 보안 헤더 설정 완료
  - CSP는 동적 콘텐츠와 충돌로 제외
- [x] 의존성 스캔: `npm audit`/`pnpm audit`, GitHub Advanced Security/CodeQL 활성화
...
- [x] 단위/통합 테스트: Jest `--ci` 그린 상태 유지 (203 passed, 일부 테스트는 구조 변경으로 인해 skip 처리)
...
- [x] GitHub Actions 활성화: `.github/workflows/*` 파이프라인에서 아래 수행
  - [x] Install: `npm ci` (또는 `pnpm i --frozen-lockfile`)
...
- `DEPLOYMENT_GUIDE.md` - 상세한 배포 가이드 및 체크리스트
- `.github/workflows/README.md` - CI/CD 워크플로 설명

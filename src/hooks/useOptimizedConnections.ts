...
export function useOptimizedConnections() {
  ...
  const fetchData = useCallback(async () => {
    const [clientsResult, itemsResult, connectionsResult] = await Promise.all([
      run(() => SupabaseHelpers.fetchAll<Client>('clients').then(res => res.data || []), { context: 'Fetch clients' }),
      run(() => SupabaseHelpers.fetchAll<Instrument>('instruments').then(res => res.data || []), { context: 'Fetch items' }),
      run(() => SupabaseHelpers.fetchAll<ClientInstrument>('client_instruments', {
        select: '*, client:clients(*), instrument:instruments(*)'
      }).then(res => res.data || []), { context: 'Fetch connections' })
    ])

    if (clientsResult) setClients(clientsResult as Client[])
    if (itemsResult) setItems(itemsResult as Instrument[])
    if (connectionsResult) setConnections(connectionsResult as ClientInstrument[])
  }, [run])
  ...
  const createConnection = async (
    ...
    return await withLoading(
      async () => {
        const { data, error } = await SupabaseHelpers.create<ClientInstrument>('client_instruments', {
          client_id: clientId,
  ...
  const deleteConnection = async (connectionId: string) => {
    return await withLoading(
      async () => {
        const { error } = await SupabaseHelpers.delete('client_instruments', connectionId)
        if (error) throw error

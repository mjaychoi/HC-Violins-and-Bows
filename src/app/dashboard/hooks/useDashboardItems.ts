...
export function useDashboardItems() {
  ...
  // Item images state
  const { data: itemImages, setItems: setItemImages, addItem: addItemImage, removeItem: removeItemImage } = useDataState<InstrumentImage>((item) => item.id, [])
  const [uploadingImages, setUploadingImages] = useState(false)
  const [imagesToDelete, setImagesToDelete] = useState<string[]>([])
  
  // Client relationships state
  const { data: clientRelationships, setItems: setClientRelationships, addItem: addClientRelationship, removeItem: removeClientRelationship } = useDataState<ClientInstrument>((item) => item.id, [])
  ...
  const fetchItemsWithClients = async () => {
    try {
      ...
      logError('Error fetching client relationships', error, 'useDashboardItems')
    }
  }

  const createItem = async (itemData: Omit<Instrument, 'id' | 'created_at'>) => {
    setSubmitting(true)
    try {
      const { data, error } = await supabase
        .from('instruments')
  ...
  const updateItem = async (id: string, itemData: Partial<Instrument>) => {
    setSubmitting(true)
    try {
      const { data, error } = await supabase
        .from('instruments')

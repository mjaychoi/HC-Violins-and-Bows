"use client"
// src/app/clients/components/ClientModal.tsx
import { useState, useEffect } from 'react'
import { Client, Instrument, ClientInstrument } from '@/types'
import { getTagColor, sortTags, formatClientName, formatClientContact } from '../utils'
...
interface ClientModalProps {
  ...
  onAddInstrument: (instrumentId: string, relationshipType: ClientInstrument['relationship_type']) => Promise<void>
...
export default function ClientModal({
  ...
  return (
    <div className="fixed right-0 top-0 h-full w-96 bg-white shadow-xl z-40 border-l border-gray-200" role="dialog" aria-modal="true" aria-labelledby="client-modal-title">
      <div className="h-full flex flex-col">
        {/* Header */}
        <div className="p-6 border-b border-gray-200">
          <div className="flex justify-between items-center">
            <h3 id="client-modal-title" className="text-lg font-medium text-gray-900">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-lg text-gray-900">{formatClientName(client)}</p>
                </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  <h4 className="text-sm font-medium text-gray-500">Instrument Connections</h4>
                  <button
                        ...
                        {searchResults.map((instrument) => (
                          <div key={instrument.id} className="p-2 hover:bg-gray-50 border-b border-gray-100 last:border-b-0">
                            <div className="flex justify-between items-center">
                              ...
                              <button
                                onClick={() => onAddInstrument(instrument.id, 'Interested')}
                ...
                {instrumentRelationships.length > 0 ? (
                  <div className="space-y-2">
                    {instrumentRelationships.map((relationship) => (
                      <div key={relationship.id} className="flex justify-between items-center bg-gray-50 p-3 rounded-md">
                        <div>
                          <div className="text-sm font-medium">
                            {relationship.instrument?.maker} - {relationship.instrument?.type}
                          </div>
                          <div className="text-xs text-gray-500">
                            {relationship.instrument?.year} • {relationship.relationship_type}
                          </div>
                        </div>
                        <button
                  ...
                  <p className="text-gray-500 text-sm">No instrument connections</p>
                )}
              </div>
            </div>
          )}

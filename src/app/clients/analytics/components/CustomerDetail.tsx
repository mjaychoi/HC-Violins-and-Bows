...
export function CustomerDetail({ customer }: { customer: CustomerWithPurchases | null }) {
  // FIXED: Normalize tags to prevent runtime crash
  const tags = customer ? (Array.isArray(customer.tags) ? customer.tags : []) : [];
  if (!customer) {
    return (
      <div className="bg-white rounded-lg shadow border border-gray-200">
        <div className="px-6 py-16 text-center">
          ...
          <p className="text-sm font-medium text-gray-900 mb-1">Select a customer to view details</p>
          <p className="text-xs text-gray-500">Choose a customer from the list to see their information</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow border border-gray-200">
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex items-start justify-between">
          ...
          <div className="text-right ml-4">
            ...
            <div className="font-mono text-sm text-gray-900 mb-3">{clientNumber || 'N/A'}</div>
      ...
      <div className="px-6 py-4 space-y-4">
        ...
        <div>
          <div className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Note</div>
          <div className="text-sm text-gray-800 whitespace-pre-wrap">{customer.note || '—'}</div>
        </div>
      </div>
    </div>
  );

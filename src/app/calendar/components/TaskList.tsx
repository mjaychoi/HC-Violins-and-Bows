          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                {/* Header */}
                {(() => {
                  // FIXED: Calculate dateStatus once and reuse for both dot and pill
                  const dateStatus = getDateStatus(task);
                  return (
                    <div className="flex items-start gap-3 mb-3">
                      <div className="shrink-0 mt-1">
                        {/* Status indicator dot - uses same logic as status pills for consistency */}
                        <div
                          className={`w-2 h-2 rounded-full ${getStatusDotClasses(
                            task.status,
                            {
                              isOverdue: dateStatus.status === 'overdue',
                              isUpcoming: dateStatus.status === 'upcoming',
                            }
                          )}`}
                        />
                      </div>
                            ...
                            {inlineEditPriority.editingId === task.id ? (
                              <div className="flex items-center gap-2">
                                <InlineSelectField<TaskPriority>
                                  ...
                                  value={task.priority}
                                  onChange={value =>
                                    inlineEditPriority.updateField(
                                      'priority',
                                      value
                                    )
                                  }
                              ...
                              <span
                                className={cn(
                                  ...
                                  getPriorityPillClasses(task.priority)
                                )}
                                onClick={e => {
                                  e.stopPropagation();
                            ...
                            {inlineEditStatus.editingId === task.id ? (
                              ...
                              <span
                                className={cn(
                                  ...
                                  getStatusColorClasses(task.status, {
                                    isOverdue: dateStatus.status === 'overdue',
                                    isUpcoming:

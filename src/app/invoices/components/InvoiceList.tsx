...
function InvoiceList({
  ...
  if (invoices.length === 0 && partial) {
    return (
      <div className="space-y-4">
        ...
        <EmptyState
          title="Invoices exist, but none on this page could be displayed"
          description={
            droppedCount > 0
              ? `${droppedCount} invoice${droppedCount === 1 ? '' : 's'} were skipped because they could not be rendered.`
              : 'Invoice records exist on this page, but they could not be rendered.'
          }
  ...
  return (
    <div className="space-y-4">
      {partial && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          {warning || 'Some invoices could not be displayed.'} Showing{' '}
          {returnedCount} invoice{returnedCount === 1 ? '' : 's'};{' '}
          {droppedCount} {droppedCount === 1 ? 'was' : 'were'} skipped.
        </div>
      )}

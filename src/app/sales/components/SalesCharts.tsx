...
export default function SalesCharts({ 
  ...
  const weekdayData = useMemo(() => {
    const weekdayMap = new Map<number, { revenue: number; refunds: number; count: number }>();
  ...
  const monthlyData = useMemo(() => {
    const monthlyMap = new Map<string, { revenue: number; refunds: number; count: number }>();
    ...
    return Array.from(monthlyMap.entries())
      .map(([key, data]) => {
        ...
        const refundRate = data.revenue > 0 ? (data.refunds / data.revenue) * 100 : 0;
  ...
  const instrumentTypeData = useMemo(() => {
    const typeMap = new Map<string, { revenue: number; count: number; refunds: number }>();
  ...
  const makerData = useMemo(() => {
    const makerMap = new Map<string, { revenue: number; count: number; refunds: number }>();
  ...
  const refundByClient = useMemo(() => {
    const clientMap = new Map<string, { refunds: number; count: number; clientName: string }>();
    ...
    filteredSales
      ...
      .forEach(sale => {
        ...
        const clientName = sale.client
          ? `${sale.client.first_name || ''} ${sale.client.last_name || ''}`.trim() || sale.client.email || 'Unknown'
          : 'Unknown';
  ...
  const refundByInstrument = useMemo(() => {
    const instrumentMap = new Map<string, { refunds: number; count: number; instrumentInfo: string }>();
    ...
    filteredSales
      ...
      .forEach(sale => {
        ...
        const instrumentInfo = sale.instrument
          ? `${sale.instrument.maker || ''} ${sale.instrument.type || ''} ${sale.instrument.subtype || ''}`.trim() || 'Missing instrument info'
          : 'Missing instrument info';
        
        if (!instrumentMap.has(instrumentId)) {
          instrumentMap.set(instrumentId, { refunds: 0, count: 0, instrumentInfo });
        }
  ...
  const handleChartClick = (
    ...
    const payload = (data as { activePayload?: Array<{ payload?: { fullDate?: string; monthKey?: string } }>; activeLabel?: string })?.activePayload;
  ...
  if (filteredSales.length === 0) {
    return (
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
        <p className="text-sm text-gray-500 text-center">No sales data available for charts</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      {dailyData.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-gray-500">Click to filter by date</span>
          </div>
      ...
      {weekdayData.some(d => d.count > 0) && (
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={weekdayData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }} barCategoryGap="20%">
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
              ...
              <YAxis
                ...
                style={{ fontSize: '12px' }}
                tickFormatter={formatCurrencyCompact}
                label={{ value: 'Amount ($)', angle: -90, position: 'insideLeft', style: { textAnchor: 'middle', fontSize: '12px' } }}
              />
              <YAxis
                yAxisId="right"
                orientation="right"
                ...
                label={{ value: 'Orders', angle: -90, position: 'insideRight', style: { textAnchor: 'middle', fontSize: '12px' } }}
              />
              <Tooltip
                formatter={(value: number, name: string) => {
                  if (name === 'Orders') return value;
              ...
              <Bar yAxisId="left" dataKey="net" fill="#3b82f6" name="Net Sales" radius={[4, 4, 0, 0]} />
              <Line
                yAxisId="right"
                type="monotone"
                dataKey="count"
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {overallRefundRate > 0 && (
              <div className="text-sm text-gray-600">
                Overall Refund Rate: <span className="font-semibold text-red-600">{overallRefundRate}%</span>
              </div>
            )}
          </div>
          <ResponsiveContainer width="100%" height={350}>
            ...
            >
              ...
              <YAxis
                ...
                domain={[0, 'auto']}
                label={{ value: 'Refund Rate (%)', angle: -90, position: 'insideRight', offset: 50, style: { textAnchor: 'middle', fontSize: '11px' } }}
              />
              <Tooltip
                formatter={(value: number, name: string) => {
                  if (name === 'Orders') return value;
              ...
              <Legend />
              <Bar yAxisId="left" dataKey="revenue" fill="#10b981" name="Revenue" radius={[4, 4, 0, 0]} />
              <Bar yAxisId="left" dataKey="refunds" fill="#ef4444" name="Refunds" radius={[4, 4, 0, 0]} />
              <Bar yAxisId="left" dataKey="net" fill="#3b82f6" name="Net Sales" radius={[4, 4, 0, 0]} />
              <Line
                yAxisId="right"
                type="monotone"
      ...
      {refundByClient.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Refunds by Client (Top 10)</h3>
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={refundByClient} layout="vertical" margin={{ top: 5, right: 30, left: 110, bottom: 70 }} barCategoryGap="0%">
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
              <XAxis 
                type="number" 
                stroke="#6b7280" 
                style={{ fontSize: '12px' }} 
                tickFormatter={formatCurrencyCompact}
                label={{ value: 'Refund Amount ($)', position: 'insideBottom', offset: -55, style: { textAnchor: 'middle', fontSize: '12px' } }}
              />
              <YAxis 
                dataKey="clientName" 
                type="category" 
              ...
              />
              <Legend wrapperStyle={{ paddingTop: '40px', marginBottom: '10px' }} />
              <Bar 
                dataKey="refunds" 
                fill="#ef4444" 
                name="Refund Amount" 
      ...
      {refundByInstrument.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Refunds by Instrument (Top 10)</h3>
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={refundByInstrument} layout="vertical" margin={{ top: 5, right: 30, left: 110, bottom: 70 }} barCategoryGap="0%">
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
              <XAxis 
                type="number" 
                stroke="#6b7280" 
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <h3 className="text-lg font-semibold text-gray-900">Top Instruments by Revenue</h3>
              <div className="group relative">
                <svg className="w-4 h-4 text-gray-400 cursor-help" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-label="Net Sales definition">
            ...
            <span className="text-xs text-gray-500">Sorted by revenue (hover for details)</span>
          </div>
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={instrumentTypeData} layout="vertical" margin={{ top: 5, right: 30, left: 70, bottom: 70 }} barCategoryGap="10%" barSize={40}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
              <XAxis 
                type="number" 
                stroke="#6b7280" 
                style={{ fontSize: '12px' }} 
                tickFormatter={formatCurrencyCompact}
                label={{ value: 'Revenue ($)', position: 'insideBottom', offset: -55, style: { textAnchor: 'middle', fontSize: '12px' } }}
              />
              <YAxis dataKey="type" type="category" stroke="#6b7280" style={{ fontSize: '12px' }} width={60} tick={{ fontSize: 12 }} />
              <Tooltip
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3">
                      ...
                      <div className="space-y-1 text-sm">
                        ...
                        <div className="flex justify-between gap-4">
                          ...
                          <span className="font-medium text-blue-600">{formatCurrency(data.net)}</span>
                        </div>
                        <div className="flex justify-between gap-4">
                          <span className="text-gray-600">Orders:</span>
                          <span className="font-medium text-gray-700">{formatNumber(data.count)}</span>
                        </div>
                        {data.refundRate > 0 && (
                          <div className="flex justify-between gap-4">
                            <span className="text-gray-600">Refund Rate:</span>
                            <span className="font-medium text-orange-600">{data.refundRate}%</span>
                          </div>
                        )}
                      </div>
                    </div>
      ...
      {makerData.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <h3 className="text-lg font-semibold text-gray-900">Top Makers by Revenue</h3>
              <div className="group relative">
                <svg className="w-4 h-4 text-gray-400 cursor-help" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-label="Net Sales definition">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
          ...
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={makerData} layout="vertical" margin={{ top: 5, right: 30, left: 90, bottom: 70 }} barCategoryGap="10%" barSize={40}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
              <XAxis 
                type="number" 
                stroke="#6b7280" 

...
import { subDays, startOfDay, endOfDay, isWithinInterval, differenceInCalendarDays } from 'date-fns';
...
export default function SalesInsights({ sales, fromDate, toDate }: SalesInsightsProps) {
  ...
  const currentPeriod = useMemo(() => {
    ...
    const refunds = currentSales.filter(s => s.sale_price < 0).reduce((sum, s) => sum + Math.abs(s.sale_price), 0);
    const netRevenue = revenue - refunds;
    const orderCount = positiveSales.length;
    const uniqueClients = new Set(positiveSales.map(s => s.client_id).filter(Boolean)).size;
  ...
  const previousPeriod = useMemo(() => {
    ...
    try {
      ...
      const revenue = positiveSales.reduce((sum, s) => sum + s.sale_price, 0);
      const refunds = prevSales.filter(s => s.sale_price < 0).reduce((sum, s) => sum + Math.abs(s.sale_price), 0);
      const netRevenue = revenue - refunds;
      const orderCount = positiveSales.length;
      const uniqueClients = new Set(positiveSales.map(s => s.client_id).filter(Boolean)).size;
      const avgTicket = orderCount > 0 ? revenue / orderCount : 0;
  ...
  const momGrowth = useMemo(() => {
    ...
    const orderGrowth = previousPeriod.orderCount > 0
      ...
      : null;
    const clientGrowth = previousPeriod.uniqueClients > 0
      ? ((currentPeriod.uniqueClients - previousPeriod.uniqueClients) / previousPeriod.uniqueClients) * 100
      : null;

    return {
      revenue: Math.round(revenueGrowth * 10) / 10,
      orders: orderGrowth !== null ? Math.round(orderGrowth * 10) / 10 : null,
      clients: clientGrowth !== null ? Math.round(clientGrowth * 10) / 10 : null,
    };
  ...
  const trend = useMemo(() => {
    ...
    const trendPercent = ((recentRevenue - previousRevenue) / previousRevenue) * 100;
    return {
  ...
  const customerVsTicketAnalysis = useMemo(() => {
    ...
    const clientChange = currentPeriod.uniqueClients - previousPeriod.uniqueClients;
    ...
    return {
      ...
      ticketContribution,
      interaction,
      primaryDriver: Math.abs(clientContribution) > Math.abs(ticketContribution) ? 'clients' : 'ticket',
    };
  }, [currentPeriod, previousPeriod]);
    ...
    <div className="space-y-4">
      ...
      {momGrowth && (
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
          <h3 className="text-sm font-semibold text-gray-700 mb-3">Period-over-Period Growth</h3>
          <div className="grid grid-cols-3 gap-4">
            <div>
              <div className="text-xs text-gray-500 mb-1">Revenue</div>
              <div className={`text-lg font-semibold ${momGrowth.revenue >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                {momGrowth.revenue >= 0 ? '+' : ''}{momGrowth.revenue}%
              </div>
            </div>
            {momGrowth.orders !== null && (
              <div>
                <div className="text-xs text-gray-500 mb-1">Orders</div>
                <div className={`text-lg font-semibold ${momGrowth.orders >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {momGrowth.orders >= 0 ? '+' : ''}{momGrowth.orders}%
                </div>
              </div>
            )}
            {momGrowth.clients !== null && (
              <div>
                ...
                <div className={`text-lg font-semibold ${momGrowth.clients >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {momGrowth.clients >= 0 ? '+' : ''}{momGrowth.clients}%
                </div>
              </div>
            )}
          </div>
      ...
      {trend && (
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
          <h3 className="text-sm font-semibold text-gray-700 mb-3">Recent Trend (Last 7 Orders in Selected Range)</h3>
      ...
      {customerVsTicketAnalysis && (
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
          <h3 className="text-sm font-semibold text-gray-700 mb-3">Revenue Change Analysis</h3>
          <div className="space-y-2">
            <div className="text-xs text-gray-600">
              Revenue change: <span className={`font-semibold ${customerVsTicketAnalysis.revenueChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div>
                ...
                <div className={`font-semibold ${customerVsTicketAnalysis.clientContribution >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {currency.format(customerVsTicketAnalysis.clientContribution)}
                </div>
                <div className="text-gray-400 text-xs mt-0.5">
                  ({customerVsTicketAnalysis.clientChange >= 0 ? '+' : ''}{customerVsTicketAnalysis.clientChange} clients)
                </div>
              </div>
              <div>
                <div className="text-gray-500 mb-1">From Avg. Ticket:</div>
                <div className={`font-semibold ${customerVsTicketAnalysis.ticketContribution >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {currency.format(customerVsTicketAnalysis.ticketContribution)}
                </div>
                <div className="text-gray-400 text-xs mt-0.5">
                  ({customerVsTicketAnalysis.ticketChange >= 0 ? '+' : ''}{currency.format(customerVsTicketAnalysis.ticketChange)})
                </div>
              </div>
            </div>
            <div className="pt-2 border-t border-gray-100">
              <div className="text-xs text-gray-600">
                Primary driver: <span className="font-semibold text-gray-900">
                  {customerVsTicketAnalysis.primaryDriver === 'clients' ? 'Customer count change' : 'Average ticket change'}
                </span>
              </div>
            </div>
          </div>

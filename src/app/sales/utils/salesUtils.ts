...
export function enrichSales(
  ...
  return sales.map(sale => ({
    ...
    instrument: sale.instrument_id ? instrumentMap.get(sale.instrument_id) : undefined,
  })) as EnrichedSale[];
}
 ...
 * 검색어로 판매 데이터를 필터링합니다.
 */
export function filterSalesBySearch(sales: EnrichedSale[], search: string): EnrichedSale[] {
 ...
 * 클라이언트 이름으로 정렬합니다.
 */
export function sortByClientName(sales: EnrichedSale[], direction: 'asc' | 'desc'): EnrichedSale[] {
  return [...sales].sort((a, b) => {
    const aValue = a.client
      ? `${a.client.first_name || ''} ${a.client.last_name || ''}`.trim() || a.client.email || ''
      : '';
...
export function calculateTotals(sales: EnrichedSale[]): SalesTotals {
  ...
  const refund = sales.reduce((sum, sale) => (sale.sale_price < 0 ? sum + Math.abs(sale.sale_price) : sum), 0);
  const avgTicket = positiveSales.length ? revenue / positiveSales.length : 0;
  const totalSales = revenue + refund;
  const refundRate = totalSales > 0 ? Math.round((refund / totalSales) * 100 * 10) / 10 : 0;
...
export function formatPeriodInfo(from: string, to: string): string {
  if (from && to) {
    try {
      ...
      if (days <= 1) {
        return fromDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', timeZone: 'UTC' });
      } else if (days <= 7) {
        return `${days} days`;
      } else if (months < 1) {
      ...
      } else if (months === 1 && fromDate.getUTCDate() === 1 && toDate.getUTCDate() === new Date(Date.UTC(toDate.getUTCFullYear(), toDate.getUTCMonth() + 1, 0)).getUTCDate()) {
        return fromDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric', timeZone: 'UTC' });
      } else {
 ...
 */
export function checkDataQuality(sales: EnrichedSale[], totalCount?: number): DataQuality {
  const positiveSales = sales.filter(sale => sale.sale_price > 0);
  
  // totalCount가 제공되면 전체 데이터 수를 기준으로 판단 (페이지네이션된 경우)
  // totalCount가 없거나 sales.length가 totalCount와 같으면 전체 데이터로 간주
  const effectiveCount = totalCount !== undefined ? totalCount : positiveSales.length;
  ...
  if (positiveSales.length > 0) {
    // 평균을 한 번만 계산 (O(n))
    const avg = positiveSales.reduce((sum, s) => sum + s.sale_price, 0) / positiveSales.length;

    // 평균이 0이 아닐 때만 outlier 체크 (모든 가격이 0인 경우 스킵)
    // Outlier 체크: 전체 데이터가 많으면 outlier가 정상적일 수 있으므로 더 관대하게
    if (avg > 0 && (isFullDataset ? positiveSales.length >= 10 : effectiveCount >= 20)) {
      // 전체 데이터인 경우 10배, 샘플인 경우 20배 이상 차이일 때만 outlier로 간주
      const outlierThreshold = isFullDataset ? 10 : 20;
      hasOutliers = positiveSales.some(sale =>
        Math.abs(sale.sale_price - avg) > avg * outlierThreshold
      );
    }
    ...
    if (dates.length < 2) {
      ...
      hasSparseDates = effectiveCount < minCountForSparse && days > 0 && dates.length / days < sparseThreshold;
    }
  }
 ...
 */
export function getDateRangeFromPreset(preset: DatePreset): { from: string; to: string } {
 ...
 * FIXED: Use parseYMDUTC to avoid timezone issues
 */
export function generateCSV(sales: EnrichedSale[], dateFormat: Intl.DateTimeFormat, currency: Intl.NumberFormat) {
  const headers = ['Date', 'Sale ID', 'Client Name', 'Client Email', 'Instrument', 'Amount', 'Status', 'Notes'];
  const csvData = sales.map(sale => ({
    // FIXED: Use parseYMDUTC instead of parseISO to avoid timezone shifts
    Date: dateFormat.format(parseYMDUTC(sale.sale_date)),
    'Sale ID': sale.id,
    'Client Name': sale.client
      ? `${sale.client.first_name || ''} ${sale.client.last_name || ''}`.trim() || 'N/A'
      : 'N/A',
    'Client Email': sale.client?.email || 'N/A',
    Instrument: sale.instrument
      ? `${sale.instrument.maker || ''} ${sale.instrument.type || ''} ${sale.instrument.subtype || ''}`.trim() || 'N/A'
      : 'N/A',
 ...
 * FIXED: Use parseYMDUTC to avoid timezone issues
 */
export function generateReceiptEmail(sale: EnrichedSale, dateFormat: Intl.DateTimeFormat, currency: Intl.NumberFormat) {
  const clientName = sale.client
  ...
  const subject = encodeURIComponent(`Receipt for Sale #${sale.id.slice(0, 8)}`);
